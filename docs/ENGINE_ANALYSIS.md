...
| ACT-02 | Pre-resolution intervention | **Met** | **[NEW · CLOSED]** An action now has a moment between being chosen and being true. `actionDeclared` is an ordinary event whose handler reads a verdict — continue, cancel, redirect or replace — and emits the real action, a retargeted one, or nothing. No handler is ever skipped and no event dropped, so the continuation invariant is untouched; it is now pinned as a property rather than by example. Cost commits at declaration, redirects are revalidated against the real targeting authority, and forced movement is deliberately not declarable. `duelistFrame` is the vertical slice, and the enemy roster carries the same kit. See `docs/INTERVENTION.md`. |
| AI-03 | Out-of-turn decision policy | **Met** | **[NEW · CLOSED]** The reaction chooser was a placeholder that accepted anything affordable. It now scores in expected HP: what the reaction achieves, what an intervention prevents, minus scarcity — linear in what is left in the pool afterwards, so the last Command Point costs four times the first. Deterministic, forecast-driven, never reads the RNG. |
| TIM-02 | Command-level turn reordering | **Met** | **[NEW · CLOSED]** A commander can permute allied activations that are already scheduled, without creating, duplicating, refunding or deleting a turn. Implemented as a view over time rather than a timestamp rewrite, because permuting `nextActionTime` cannot express a permutation between units that share one — the comparator's own tie-breaks undo it silently. A plan re-deals the exact multiset of slots those units held; enemy activations are barriers; commanded slots retire when spent, when the unit dies, or when the timeline reschedules it. `battlePlan` is the vertical slice, built entirely as content. See `docs/SEQUENCING.md`. |
| RES-02 | Costs in any resource scope | **Met** | **[NEW · CLOSED]** An ability cost naming a faction-scoped resource used to deduct nothing and report nothing — `resourceSpent` only ever read the unit's own pools. Cost checking and spending are scope-aware now in the event handler, both command validators, the content validator and the ability view model, so a squad-funded action is charged to the squad. |
| SUP-01 | Resource transfer and conversion | **Met** | **[NEW · CLOSED]** One generic `transferResource` effect moves points rather than creating them, covering a hand-off, a conversion between two different resources and a personal resource contributed to a shared pool — because none of those are different sums. Scope comes from the resource definition and the owner from the from/to selector, so crossing scope needs no special case. Transfers are counted in whole exchanges, so a partial transfer spends only what it delivers. The debit and credit happen in one step, so nothing exists nowhere and no pair of clamps can disagree. See `docs/SUPPORT.md`. |
| STA-01 | Status/component framework | **Met** | **[CLOSED]** Timed statuses were already expressive enough for a damaged frame in every way but one — only *equipment* could remove an ability, and equipment does not change mid-battle. A status may now declare `removesAbilities` and `blocksAbilityTags`, which is this project's entire subsystem-damage model and deliberately six lines: an impairment inherits the whole status lifecycle, and repair is an ordinary `cleanse`. A dedicated subsystem model would have reimplemented duration, stacking, display and save/load to gain nothing. |
| FIX-01 | Persistent tile-anchored entities | **Met** | **[NEW · CLOSED]** A generic battlefield-fixture model: anchored to a tile, owned by a faction, stateful, serializable, non-occupying by default, and not a unit. Triggers fire from ordinary movement events, so voluntary and forced movement reach them identically and a mid-path crossing sets a device off where it is. Detonation is a fixture-targeted command selecting by tag, never by id. See `docs/FIXTURES.md`. |
| TGT-01 | Propagation targeting | **Met** | **[NEW · CLOSED]** A bounded chain that walks from target to target over live positions, asking the engine's own distance, sight and filter authorities at every step. Selection policies are a registry, arc cost and reach are authored, and hop-dependent damage rides the same named-scaling sources as approach distance. The preview and execution share one planner and are compared by test. `cascadeFrame` is the vertical slice, built entirely as content. See `docs/PROPAGATION.md`. |
| MOV-02 | Route/trajectory movement | **Met** | **[NEW · CLOSED]** Segments, categorised redirects, mid-route contacts and precise displacement, planned through the engine's own step validator so there is no second pathfinder and no second legality model. The player's preview, execution and the tests all call `planUnitTrajectory`. Turn cost is authored, not coded. `interdictorFrame` is the vertical slice built entirely as content. See `docs/TRAJECTORY.md`. |
...
| POS-01 | Precise forced positioning | **Met** | **[NEW · CLOSED]** An authored `displace` distance is a *maximum*; the player chooses the exact distance and heading and the engine clamps it. `displacementResistance` shortens the shove before the walk, and the resulting event publishes requested / resisted / actual / blocked-by so a reaction can respond to the shove that fell short. A shove that moves nobody still emits `displacementBlocked`. |
...
| REG-01 | Exact region membership | **Met** | **[NEW · CLOSED]** One authoritative helper (`src/mission/regions.js`) answers whether a tile is in a region, used by objectives, the scripting layer and the engine alike. The mid-battle trigger's bounding-box approximation is gone, and with it the two standing "not rectangular" warnings. |
